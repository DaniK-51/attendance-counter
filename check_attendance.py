...
def check_attendance(teams_csv: Path, attendance_txt: Path) -> str:
    ...
    for team, members in sorted(teams.items(), key=lambda kv: team_sort_key(kv[0])):
        ...
        out.write(f"{team:12s}  {count:3d}/{len(members):3d}  ({pct:5.1f}%)\n")
    overall_pct = total_present / total_members * 100 if total_members else 0
    out.write("-" * 60 + "\n")
    out.write(f"{'TOTAL':12s}  {total_present:3d}/{total_members:3d}  ({overall_pct:5.1f}%)\n")

    # ── Absent students ──
    absent = sorted(all_members - attended, key=lambda e: (team_sort_key(email_to_team[e]), e))
    out.write(f"\n{'=' * 60}\n")
    out.write(f"ABSENT STUDENTS ({len(absent)})\n")
    out.write("=" * 60 + "\n")
    current_team = None
    for email in absent:
        ...
        if team != current_team:
            if current_team is not None:
                out.write("\n")
            current_team = team
        out.write(f"  {email:45s}  {team}\n")

    # ── Unknown students ──
    unknown = sorted(attended - all_members)
    out.write(f"\n{'=' * 60}\n")
    out.write(f"UNKNOWN STUDENTS — in attendance but NOT in teams ({len(unknown)})\n")
    out.write("=" * 60 + "\n")
    ...
    if corrupted:
        out.write(f"\n{'=' * 60}\n")
        out.write(f"CORRUPTED EMAILS ({len(corrupted)})\n")
        out.write("=" * 60 + "\n")
...
if __name__ == "__main__":
    ...
    output = check_attendance(TEAMS_CSV, attendance_file)

    output_path = RATING_DIR / f"{stem}_report.txt"
